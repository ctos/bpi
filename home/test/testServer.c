	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	OutP->RetCode = send(In0P->Head.msgh_request_port, In0P->num);
}

/* Routine receive */
mig_internal void _Xreceive
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	typedef struct {
		mach_msg_header_t Head;
	...
	OutP->RetCode = receive(In0P->Head.msgh_request_port, &OutP->num);
...
static mig_routine_t test_server_routines[] = {
		_Xsend,
		_Xreceive,
};

mig_external boolean_t test_server
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	if ((InP->msgh_id > 438) || (InP->msgh_id < 437) ||
	    ((routine = test_server_routines[InP->msgh_id - 437]) == 0)) {
...
mig_external mig_routine_t test_server_routine
	(const mach_msg_header_t *InHeadP)
{
	register int msgh_id;

	msgh_id = InHeadP->msgh_id - 437;

	if ((msgh_id > 1) || (msgh_id < 0))
		return 0;

	return test_server_routines[msgh_id];
}
