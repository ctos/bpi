	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	mig_external kern_return_t Cache_send_message
		(mach_port_t send_port, string_t message);
	...
	OutP->RetCode = Cache_send_message(In0P->Head.msgh_request_port, In0P->message);
}
	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	register Request *In0P = (Request *) InHeadP;
	register Reply *OutP = (Reply *) OutHeadP;
	mig_external kern_return_t Cache_get_message
		(mach_port_t recv_port, string_t message);
	...
	OutP->RetCode = Cache_get_message(In0P->Head.msgh_request_port, OutP->message);
...
static mig_routine_t Cache_cache_server_routines[] = {
		_Xsend_message,
		_Xget_message,
};

mig_external boolean_t Cache_cache_server
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	if ((InP->msgh_id > 4067) || (InP->msgh_id < 4066) ||
	    ((routine = Cache_cache_server_routines[InP->msgh_id - 4066]) == 0)) {
...
mig_external mig_routine_t Cache_cache_server_routine
	(const mach_msg_header_t *InHeadP)
{
	register int msgh_id;

	msgh_id = InHeadP->msgh_id - 4066;

	if ((msgh_id > 1) || (msgh_id < 0))
		return 0;

	return Cache_cache_server_routines[msgh_id];
}
