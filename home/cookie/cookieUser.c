...
)
{
	...
	auto const mach_msg_type_t send_messageType = {
	...
	InP->send_messageType = send_messageType;

	(void) mig_strncpy(InP->send_message, send_message, 1024);
...
)
{
	...
	auto const mach_msg_type_t recv_messageCheck = {
		/* msgt_name = */		MACH_MSG_TYPE_STRING_C,
		/* msgt_size = */		8,
		/* msgt_number = */		1024,
		/* msgt_inline = */		TRUE,
...
#if	TypeCheck
	if (BAD_TYPECHECK (&OutP->recv_messageType, &recv_messageCheck))
		return MIG_TYPE_ERROR;
#endif	/* TypeCheck */

	(void) mig_strncpy(recv_message, OutP->recv_message, 1024);
